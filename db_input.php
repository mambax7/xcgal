...
if($_FILES['userpicture1']['tmp_name'] != "") {
        ...
        $keywords = $myts->makeTboxData4Save($_POST['keywords1']);
        ...
        // Check image type is among those allowed for ImageMagick
                ...
                // Create thumbnail and internediate image and add the image into the DB
                $result = add_picture($album, $filepath, $picture_name, $title, '', $keywords, '', '', '', '', $category);
                if(!$result){
                        @unlink($uploaded_pic);
                        redirect_header('index.php',2,sprintf(_MD_DB_ERR_INSERT, $uploaded_pic).'<br /><br />'.$ERROR);
                } elseif ($PIC_NEED_APPROVAL) {
...
if($_FILES['userpicture2']['tmp_name'] != "") {
        ...
        $keywords = $myts->makeTboxData4Save($_POST['keywords2']);
...
if($_FILES['userpicture3']['tmp_name'] != "") {
        ...
        $keywords = $myts->makeTboxData4Save($_POST['keywords3']);
...
if($_FILES['userpicture4']['tmp_name'] != "") {
        ...
        $keywords = $myts->makeTboxData4Save($_POST['keywords4']);
