...
function display_thumbnails($album, $cat, $page, $thumbcols, $thumbrows, $display_tabs)
{
        ...
        if (count($pic_data) > 0) {
                foreach ($pic_data as $key => $row) {
                        ...
                        $thumb_list[$i]['image'] = "<img src=\"" . get_pic_url($row, 'thumb') . "\" class=\"image\" {$image_size['geom']} border=\"0\" alt=\"{$album_name} - {$row['filename']}\" title=\"$pic_title\" />";     # deleted </a> at end mcleines
